...
import {
  Layout, Tabs, Table, Button, Space, Tag, Input, Select, DatePicker,
...
const mockOrders: RectOrder[] = [
  { id: 'REC-20260101', title: '采购合同审批流程缺陷整改', department: '采购部', source: 'AUD-20250001',
    ...
    evidence: ['流程截图.png', '整改方案.docx'], verifyStatus: 'pending' },
  { id: 'REC-20260102', title: '费用报销附件不完整整改', department: '财务部', source: 'AUD-20250001',
    ...
    evidence: ['补传清单.xlsx'], verifyStatus: 'pending' },
  ...
  { id: 'REC-20260104', title: '供应商准入资质审查整改', department: '采购部', source: 'AUD-20250002',
    ...
    evidence: ['营业执照.pdf', '资质清单.xlsx'], verifyStatus: 'passed' },
  { id: 'REC-20260105', title: '销售合同毛利率异常整改', department: '销售部', source: 'AUD-20250004',
    ...
    evidence: ['定价分析.xlsx'], verifyStatus: 'pending' },
  ...
  { id: 'REC-20260107', title: '差旅标准超标整改', department: '财务部', source: 'AUD-20250006',
    ...
    evidence: ['制度修订版.pdf'], verifyStatus: 'passed' },
];
...
const RectificationPage: React.FC = () => {
  ...
  }, [orders, searchText, statusFilter, severityFilter]);
  ...
  const stats = useMemo(() => ({
    ...
    completed: orders.filter(o => o.status === 'verified' || o.status === 'closed').length,
  }), [orders]);
  ...
  const columns: ColumnsType<RectOrder> = [
    ...
    { title: '进度', dataIndex: 'progress', key: 'progress', width: 130,
      render: (v: number) => <Progress percent={v} size="small" strokeColor={v >= 100 ? '#52c41a' : '#E34D59'} /> },
  ...
  const barOption = {
    ...
    xAxis: { type: 'category', data: ['采购部', '财务部', '行政部', '销售部', 'IT部'], axisLabel: { fontSize: 11 } },
    yAxis: { type: 'value', axisLabel: { fontSize: 11 }, nameTextStyle: { fontSize: 10 } },
    series: [
      { name: '已完成', type: 'bar', stack: 'total', data: [2, 3, 1, 1, 0], color: '#52c41a' },
      { name: '进行中', type: 'bar', stack: 'total', data: [1, 1, 0, 1, 2], color: '#1890ff' },
      { name: '逾期', type: 'bar', stack: 'total', data: [0, 1, 0, 0, 1], color: '#f5222d' },
      { name: '待派发', type: 'bar', stack: 'total', data: [1, 0, 1, 0, 0], color: '#d9d9d9' },
    ],
  };
  ...
  const pieOption = {
    ...
    series: [{
      ...
      data: [
        ...
        { value: orders.filter(o => o.status === 'in_progress').length, name: '进行中', itemStyle: { color: '#1890ff' } },
        { value: stats.overdue, name: '逾期', itemStyle: { color: '#f5222d' } },
        { value: stats.pending, name: '待派发', itemStyle: { color: '#d9d9d9' } },
      ],
      label: { formatter: '{b}\n{d}%' },
    }],
  };
  ...
  const lineOption = {
    ...
    yAxis: { type: 'value', name: '完成率%', axisLabel: { fontSize: 11 }, nameTextStyle: { fontSize: 10 }, min: 0, max: 100 },
    series: [{
      type: 'line', smooth: true, data: [45, 52, 58, 65, 72, 78],
      symbol: 'circle', symbolSize: 6,
      areaStyle: { color: 'rgba(227,77,89,0.1)' },
  ...
  return (
    <Layout>
      ...
      <div className="page-header">
        <h2 className="page-title">✅ 整改跟踪中心</h2>
        <p className="page-subtitle">整改全闭环管理：派发 → 追踪 → 验证 → 归档</p>
      </div>
      <Content className="page-content">
        <Tabs defaultActiveKey="orders" items={[
          {
            key: 'orders',
            label: <span><FileDoneOutlined /> 整改工单</span>,
            children: (
          ...
          {
            key: 'verify',
            label: <span><CheckCircleOutlined /> 验证管理</span>,
            children: (
              <div className="content-card">
                <Row gutter={16} style={{ marginBottom: 16 }}>
          ...
          {
            key: 'stats',
            label: <span><BarChartOutlined /> 统计看板</span>,
            children: (
              <div className="content-card">
                <Row gutter={16}>
            ...
            <div>
              <Descriptions column={2} bordered size="small">
                ...
                <Descriptions.Item label="证据附件" span={2}>
                  {selectedOrder.evidence?.length > 0 ? selectedOrder.evidence.map((e, i) => <Tag key={i} color="blue">{e}</Tag>) : <Text type="secondary">暂无证据</Text>}
                </Descriptions.Item>
              </Descriptions>
              <div style={{ marginTop: 16 }}>
                <Text strong>整改进度</Text>
                <Progress percent={selectedOrder.progress} strokeColor={selectedOrder.progress >= 100 ? '#52c41a' : '#E34D59'} style={{ marginTop: 8 }} />
              </div>
              <div style={{ marginTop: 16 }}>
